...
DATA_FOLDER = "/data/OCR/chinese"
OUTPUT = "/data/OCR/chinese/ctpn_labels"
MAX_LEN = 1200
MIN_LEN = 600
...
img = cv2.imread("img_810.jpg")
with open("img_810.txt") as f:
    labels = f.readlines()
rec = []
for label in labels:
    label = label.strip()
    label = label.split(",")
    rec.append([int(label[0]), int(label[1]), int(label[2]), int(label[3])])
for label in rec:
    cv2.rectangle(img, (label[0], label[1]), (label[2], label[3]), (0,255,0), 4)
