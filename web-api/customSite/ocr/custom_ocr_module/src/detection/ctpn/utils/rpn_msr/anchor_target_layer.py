...
def anchor_target_layer(rpn_cls_score, gt_boxes, im_info, _feat_stride=[16, ], anchor_scales=[16, ]):
    ...
    # 生成基本的anchor,一共10个    每个网格中心在原图对应中心的偏移量
    ...
    im_info = im_info[0]  # 原始图像的信息，图像的高宽及通道数
    if DEBUG:
    ...
    # map of shape (..., H, W)
    height, width = rpn_cls_score.shape[1:3]  # feature-map的高宽  H/16, W/16
    ...
    # 为当前的feature map每个点生成A个anchor，shape is [K,A,4]
    all_anchors = (_anchors.reshape((1, A, 4)) +
                   shifts.reshape((1, K, 4)).transpose((1, 0, 2)))  # 相当于复制宽高的维度，然后相加
    all_anchors = all_anchors.reshape((K * A, 4))                    # 共有K*A (H/16*W/16*10) anchor box  [xmin, ymin, xmax, ymax]
    ...
    argmax_overlaps = overlaps.argmax(axis=1)  # (A)#找到和每一个gtbox，overlap最大的那个anchor,  每个anchor， overlap最大的
    max_overlaps = overlaps[np.arange(len(inds_inside)), argmax_overlaps]   # 每个anchor，和各个gt的iou最大的值
    gt_argmax_overlaps = overlaps.argmax(axis=0)  # G#找到每个位置上10个anchor中与gtbox，overlap最大的那个, 和gt overlap最大
    gt_max_overlaps = overlaps[gt_argmax_overlaps,
                               np.arange(overlaps.shape[1])]
    gt_argmax_overlaps = np.where(overlaps == gt_max_overlaps)[0]      # 和每个gt重叠最大的anchor

    if not cfg.RPN_CLOBBER_POSITIVES:
        # assign bg labels first so that positive labels can clobber them
        labels[max_overlaps < cfg.RPN_NEGATIVE_OVERLAP] = 0  # 先给背景上标签，小于0.3overlap的   如果这个anchor，最大的iou<0.3, 则是背景

    # -----------------------------------#
    # 正样本的确定，iou得分大于0.7和每个位置上具有最大IOU得分的anchor
    # fg label: for each gt, anchor with highest overlap
    # 每个位置上的10个anchor中overlap最大的认为是前景
    ...
    bbox_targets = _compute_targets(anchors, gt_boxes[argmax_overlaps, :])  # 根据anchor和gtbox计算得真值（anchor和gtbox之间的偏差）    gt_boxes[argmax_overlaps, :] 各个anchor最大iou对应的gt
    ...
    labels = _unmap(labels, total_anchors, inds_inside, fill=-1)  # 这些anchor的label是-1，也即dontcare     # 所有anchor，包括超出边界的，0，1，-1，前景1， 背景0， 其余包括超出边界-1
