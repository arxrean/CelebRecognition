...
def obtain_age_gender(img_path,model,face_detect_model):
    margin = 0.4
    ...
    detected = face_detect_model(input_img, 1)
    faces = np.empty((len(detected), img_size, img_size, 3))
    ...
    if len(detected) > 0:
        ...
        return int(predicted_ages[0]), 'M' if predicted_genders[0][0] < 0.5 else 'F'
