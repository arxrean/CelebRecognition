...
class inceptblock(nn.Module):
    def __init__(self, in_channels, out_channels, scale=0.3):
        super(inceptblock, self).__init__()
        self.scale = scale
        self.branch0 = mfm(in_channels, 32, kernel_size=1, stride=1, padding=0)
        self.branch1 = nn.Sequential(
            mfm(in_channels, 32, kernel_size=1, stride=1, padding=0),
            mfm(32, 48, kernel_size=3, stride=1, padding=1)
        )
        ...
        self.conv2d = mfm(144, out_channels, kernel_size=1,
                          stride=1, padding=0)
...
class network_29layers_v3(nn.Module):
    def __init__(self, block, layers, num_classes=79077, args=None):
    ...
    def forward(self, x, target_var=None):
        ...
        if target_var is None:
            ...
            fc = l2_norm(fc)
            out = self.arcface_loss(fc, target_var, self.out_num, s=64, m=0.5)
        return out, fc
...
class network_29layers_without_arc(nn.Module):
    def __init__(self, block, layers, num_classes=79077, args=None):
        super(network_29layers_without_arc, self).__init__()
        self.out_num = num_classes
        self.conv1 = mfm(1, 48, 5, 1, 2)
        self.block1 = self._make_layer(block, layers[0], 48, 48)
        self.group1 = group(48, 96, 3, 1, 1)
        ...
        self.group4 = group(128, 128, 3, 1, 1)
        self.fc = nn.Linear(7*7*128, 256)
        self.fc2 = nn.Linear(256, self.out_num)
...
def LightCNN_29Layers_without_arc(**kwargs):
    model = network_29layers_without_arc(resblock, [1, 2, 3, 4], **kwargs)
    return model
